...
CREATE POLICY "Authenticated users can read hangout posts" ON public.hangout_posts FOR SELECT USING (auth.uid() IS NOT NULL);
...
CREATE POLICY "Authenticated users can read server messages" ON public.server_messages FOR SELECT USING (auth.uid() IS NOT NULL);
...
CREATE POLICY "Authenticated users can read spaces" ON public.spaces FOR SELECT USING (auth.uid() IS NOT NULL);
...
CREATE POLICY "Authenticated users can read channels" ON public.channels FOR SELECT USING (auth.uid() IS NOT NULL);
