...
import { migrateNoteCourseMap } from "../../lib/courseStorageMigration";
...
.ph-serif { font-family: 'DM Serif Display', serif; }
.ph-btn-primary { background: #4059FF; color: #fff; border: none; }
.ph-btn-primary:hover { background: #3450e6; }
.ph-surface { background: #fff; border: 1px solid #EBEBEB; }
`;

const COURSE_ACCENT_COLORS = ["#4059FF", "#10B981", "#F59E0B", "#8B5CF6", "#EC4899", "#14B8A6"];
...
export function Personal() {
  ...
  const getTotalNoteCountForCourse = (courseId: string): number =>
    notes.filter((note) => note.course_id === courseId).length;

  useEffect(() => {
    chatEndRef.current?.scrollIntoView({ behavior: "smooth" });
  }, [messages]);

  const coursesStorageKeyForUser = coursesStorageKey(user?.id);

  const readCachedCourses = (): PersonalCourse[] => {
    return readLocalCourses(coursesStorageKeyForUser).map((row) => ({
      id: row.id,
      code: row.code,
      title: row.title,
  ...
  useEffect(() => {
    ...
    const loadCourses = async () => {
      ...
      await migrateNoteCourseMap(user.id);

      const { courses, backendAvailable } = await syncUserCourses(user.id);
      setIsBackendAvailable(backendAvailable);

      if (cancelled) return;

      const mapped: PersonalCourse[] = courses.map((row) => ({
        id: row.id,
        code: row.code,
        title: row.title,
        subtitle: row.subtitle || "No subtitle provided",
      ...
      setCourses(mapped.length > 0 ? mapped : readCachedCourses());
    };
  ...
  return (
    <div className="ph-root flex h-full justify-center overflow-y-auto" style={{ backgroundColor: "var(--bg-main, #FAFAF8)", color: "var(--text-primary, #0E1117)" }}>
      <style>{PERSONAL_CSS}</style>
      <div className="w-full max-w-[1320px] p-3 sm:p-4 lg:p-5 min-[1440px]:p-6">
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-3 mb-4 pt-1">
          <div>
            <h1 className="ph-serif text-3xl tracking-tight mb-1" style={{ color: "#0E1117" }}>Personal Hub</h1>
            <p className="text-sm font-medium tracking-wide" style={{ color: "#6B7280" }}>Manage your academic journey and campus life.</p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            <button type="button" className="ph-surface flex items-center justify-center gap-2 px-3 py-2 text-sm font-semibold rounded-xl transition-colors w-full md:w-auto" style={{ color: "#6B7280" }}>
            ...
            <button 
              ...
              className="ph-btn-primary flex items-center justify-center gap-2 px-3 py-2 text-sm font-semibold rounded-xl shadow-md transition-colors w-full md:w-auto"
              style={{ boxShadow: "0 8px 24px rgba(64,89,255,.2)" }}
            >
