...
import { AppTag, DEFAULT_TAG_NAMES, FALLBACK_TAGS, getTagStyle, normalizeTagName } from "../../lib/tagSystem";
import { sanitizeNoteHtml } from "../../lib/utils";
import { listCoursesForUser } from "../../lib/courseSync";
...
export function Editor() {
  ...
  useEffect(() => {
    ...
    setSelectedCourseId(note.course_id ?? null);
  }, [id, note?.id, note?.course_id]);

  const selectCourse = async (cid: string | null) => {
    if (!id) return;
    setSelectedCourseId(cid);
    if (user?.id) {
      try { await updateNote(id, { course_id: cid }); } catch { /* offline */ }
    }
  ...
  useEffect(() => {
    ...
    if (editorRef.current.innerHTML !== text) editorRef.current.innerHTML = sanitizeNoteHtml(text);
  }, [id, note?.id]); // eslint-disable-line
  ...
  const handleSummarize = async () => {
    ...
    try {
      const { summarizeNote } = await import("../../lib/qwen");
  ...
  const sendAi = async () => {
    ...
    try {
      const { lumiChat } = await import("../../lib/qwen");
