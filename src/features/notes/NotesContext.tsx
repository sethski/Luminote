...
import { supabase, Note, Reminder } from "../../lib/supabaseClient";
import { useAuth } from "../auth/AuthContext";
import { ensureCourseExists } from "../../lib/courseSync";
...
export function NotesProvider({ children }: { children: ReactNode }) {
  ...
  const fetchNotes = useCallback(async (force = false, silent = false) => {
    ...
    if (error && isAuthRaceError(error)) {
      console.warn("Notes fetch auth race, retrying once...");
  ...
  const fetchReminders = useCallback(async () => {
    ...
    if (error && isAuthRaceError(error)) {
      console.warn("Reminders fetch auth race, retrying once...");
      const { data: authData } = await supabase.auth.getSession();
      if (authData.session) {
        const retry = await query();
        data = retry.data;
  ...
  const addNoteForCourse = async (courseId: string): Promise<string> => {
    ...
    const exists = await ensureCourseExists(user.id, courseId);
    if (!exists) {
  ...
  const addNoteForCourse_Internal = async (userId: string, courseId: string | null): Promise<string> => {
    const { data, error } = await supabase
      ...
      .single();
    if (error) {
      console.error("addNoteForCourse failed:", error.message);
      throw error;
    }
    const inserted = data as Note;
    setNotes(prev => [inserted, ...prev]);
  ...
  const updateNote = async (id: string, updates: Partial<Note>) => {
    ...
    try {
      ...
      if (error) {
        console.error("updateNote failed:", error.message);
  ...
  const extractRemindersFromNote = async (
    ...
    try {
      ...
      const { extractRemindersFromText } = await import("../../lib/qwen");
      ...
      if (deleteError) {
        console.error("reminder cleanup failed:", deleteError.message);
      }
