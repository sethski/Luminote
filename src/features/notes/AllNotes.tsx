...
import { lumiChat } from "../../lib/qwen";
import { getCleanPreview } from "../../lib/utils";
import { DEFAULT_TAG_NAMES, getTagStyle } from "../../lib/tagSystem";
...
.an-root { font-family: 'DM Sans', sans-serif; background: var(--bg-main, #FAFAF8); }
.an-serif { font-family: 'DM Serif Display', serif; }
`;
...
export function AllNotes() {
  ...
  return (
    <div className="an-root relative flex h-full w-full max-w-[1760px] flex-col p-4 sm:p-6 lg:p-8 min-[1440px]:p-10">
      <style>{ALL_NOTES_CSS}</style>
      
      {/* Header Area */}
      <div className="mb-6 flex flex-col gap-4 sm:mb-8 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 className="an-serif mb-1 text-2xl tracking-tight sm:text-3xl" style={{ color: "#0E1117" }}>All Notes</h1>
          <p className="font-medium" style={{ color: "#6B7280" }}>{filteredNotes.length} notes found in your library</p>
        </div>
        ...
        <div className="flex w-full flex-col items-stretch gap-3 sm:flex-row sm:items-center md:w-auto">
          <div className="relative flex-1 sm:min-w-[18rem] md:w-80">
            ...
            <input
              ...
              className="min-h-11 w-full rounded-xl border py-3 pl-11 pr-4 font-medium transition-all focus:outline-none focus:ring-2"
              style={{
                borderColor: "#EBEBEB",
                background: "#FFFFFF",
                color: "#0E1117",
              ...
              onFocus={(e) => {
                e.currentTarget.style.borderColor = "#4059FF";
                e.currentTarget.style.boxShadow = "0 0 0 3px rgba(64,89,255,.12)";
              }}
              onBlur={(e) => {
