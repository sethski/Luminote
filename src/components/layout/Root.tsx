...
function NavItem({
  ...
  return (
    <NavLink
      ...
      style={
        active
          ? { background: "rgba(64,89,255,.1)", color: "#4059FF" }
...
function MobileNavItem({
  ...
  return (
    <NavLink to={to} className="flex min-h-11 w-full flex-col items-center justify-center py-1 transition-colors" style={{ color: active ? "#4059FF" : "#9CA3AF" }}>
...
function RootInner() {
  ...
  return (
    <div className="flex h-[100dvh] min-h-0 overflow-hidden transition-colors duration-300" style={{ background: "var(--bg-main, #FAFAF8)", color: "var(--text-primary, #0E1117)", fontFamily: "'DM Sans', sans-serif" }}>
      {!hideNavigation && (
        <aside
          className="z-20 hidden w-[232px] flex-col border-r transition-colors duration-300 lg:w-[248px] xl:w-[264px] 2xl:w-[288px] md:flex"
          style={{ background: "var(--bg-card, #FFFFFF)", borderColor: "#EBEBEB", boxShadow: "4px 0 24px rgba(14,17,23,.03)" }}
        >
          <div className="flex items-center gap-3 px-6 py-6 mb-2">
            <div className="w-14 h-14 rounded-xl bg-[#0E1117] flex items-center justify-center shrink-0">
              <LuminoteLogo size={32} />
            </div>
            <span className="text-lg font-bold tracking-tight" style={{ color: "#0E1117", fontFamily: "'DM Serif Display', serif" }}>
              Luminote
            </span>
          </div>
            ...
            >
              <div className="flex items-center gap-3">
                {profile?.avatar_url ? (
                  ...
                  <div className="w-9 h-9 rounded-full flex items-center justify-center text-white text-xs font-bold" style={{ background: "#4059FF" }}>
                    {initials}
                  </div>
                )}
                <div>
      ...
      <main
        ...
        style={{ background: "var(--bg-main, #FAFAF8)" }}
      >
