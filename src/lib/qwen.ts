/**
 * qwen.ts — AI service via OpenRouter
 * Production: Supabase qwen-proxy edge function or /api/* routes.
 * Dev fallback: VITE_OPENROUTER_API_KEY (never bundled for prod builds).
 */
...
async function callQwen(messages: QwenMessage[], opts: QwenOptions = {}): Promise<string> {
  ...
  /* Direct OpenRouter fallback — dev only; never ship keys in the client bundle */
  if (!import.meta.env.DEV) {
    throw new Error("AI unavailable. Configure the qwen-proxy edge function or /api routes.");
  }
  const viteEnv = import.meta.env as Record<string, string | undefined>;
  const apiKey = viteEnv.VITE_OPENROUTER_API_KEY;
  if (!apiKey) throw new Error("Missing VITE_OPENROUTER_API_KEY in .env (dev only)");
